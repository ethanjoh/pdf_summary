...
### 변경 목적
- 사용자가 다권으로 구성된 시리즈물 중 임의의 1개 파일(예: `상도 1.pdf`, `해리포터 2권.pdf`) 경로만 지정하더라도, 같은 폴더 내에 있는 나머지 모든 시리즈 권수를 자동으로 찾아내어 하나의 완성된 시리즈 통합 서평으로 생성할 수 있도록 개선.

### 주요 결정 사항
1. **시리즈 패턴 자동 감지 및 전 권 자동 수집 (`main.py`)**:
   - `input_path`가 파일일 때 `parse_series_info`를 통해 시리즈 권수 번호(`vol_num > 0`)를 확인.
   - 시리즈인 경우 상위 폴더(`input_path.parent`) 내에서 동일 시리즈명(`base_title`)을 가진 모든 PDF 파일을 검색하여 권수 순서대로 자동 정렬/수집.
   - 단권 도서(`vol_num == 0`)인 경우 기존과 동일하게 해당 1권만 단독 처리.
2. **배너 안내 강화 (`main.py`)**:
   - 시리즈가 자동 수집된 경우 콘솔 배너에 `📚 지정 도서(시리즈): '{detected_series_title}' (동일 폴더 내 총 N권 자동 수집)`으로 명확히 시각화.
3. **단위 테스트 추가 (`test_pdf_processor.py`)**:
   - 시리즈 중 특정 권 지정 시 1~3권 전체 자동 수집 검증 단위 테스트 추가.
4. **가이드 문서 전면 개편 (`README.md`)**:
   - 스마트 시리즈 자동 수집 기능 설명 반영.
   - 단일 도서, 시리즈 도서, 폴더 일괄 실행 및 실전 추천 옵션 조합 등 사용 방법 가이드 상세화.

### 수정한 파일
- `main.py`: 시리즈 단일 파일 지정 시 동일 폴더 내 전 권 자동 수집 로직 구현
- `test_pdf_processor.py`: 시리즈 자동 수집 단위 테스트 추가
- `README.md`: 가이드 문서 갱신
- `CHANGELOG.md`: 변경 기록 추가

### 테스트 결과
- `test_pdf_processor.py` 단위 테스트 전체 정상 통과 (시리즈 자동 수집, 장편 최적화, 분할 청킹 등 전체 PASS)

---

## [2026-09-02] - 특정 단일 PDF 도서 파일 직접 지정(-i, -f) 실행 지원

### 변경 목적
- 폴더 단위 일괄 처리뿐만 아니라, 특정 단일 PDF 도서 파일의 경로를 직접 전달하여 단독으로 빠르게 분석 및 서평 생성을 진행할 수 있도록 개선.

### 주요 결정 사항
1. **단일 파일 및 폴더 경로 자동 인식 (`main.py`)**:
   - `--input`, `--input_dir`, `--file`, `-i`, `-f` 옵션에 전달된 경로가 `.pdf` 파일인 경우 해당 도서 1권만 즉시 단독 처리.
   - 단일 파일의 결과물은 파일이 속한 상위 폴더명을 기준으로 서브폴더(`output/{상위폴더명}/`)에 자동 분류 저장되어 일관된 산출물 체계 유지.
2. **단위 테스트 추가 (`test_pdf_processor.py`)**:
   - `--file` 및 `-f` 단일 파일 CLI 옵션 파싱 단위 테스트 추가.
3. **가이드 문서 갱신 (`README.md`)**:
   - 단일 PDF 파일 실행 예시 및 CLI 옵션 표 갱신.

### 수정한 파일
- `main.py`: 단일 파일/폴더 자동 감지 및 CLI 옵션 확장
- `test_pdf_processor.py`: 단일 파일 인자 단위 테스트 추가
- `README.md`: 단일 파일 실행 가이드 및 옵션 표 갱신
- `CHANGELOG.md`: 변경 기록 추가

### 테스트 결과
- `test_pdf_processor.py` 단위 테스트 전체 정상 통과
- `python main.py -h` 도움말 출력 확인

---

## [2026-09-02] - 대용량 도서 분할 쿨다운 및 장편 시리즈(3권 이상) 권별 분할 요약 최적화

### 변경 목적
- 700~1,000페이지 이상의 대용량 도서 및 3권 이상의 다권 장편 시리즈(`해리포터 4~6권`, `상도`, `잃어버린 왕국`, `Principles of Marketing` 등) 처리 시, 단일 요청 토큰 한도(1M) 초과 또는 연속 호출로 인한 분당 토큰 한도(TPM)/서버 과부하(503) 에러로 서평 생성이 실패하던 현상을 해결.

### 주요 결정 사항
1. **장편 시리즈(3권 이상 / 30만 자 초과) '권별 요약 후 통합' 자동 파이프라인 가동 (`summarizer.py`)**:
   - 3권 이상의 다권 시리즈 감지 시, 모든 텍스트를 무리하게 합쳐 1회 호출하는 대신 각 권별 핵심 요약을 1차 생성한 뒤 이를 취합하여 최종 종합 서평으로 통합.
   - 단일 요청 토큰 소모량을 1/5 수준으로 절감하여 토큰 초과 및 서버 과부하 원천 차단.
2. **분할 요약 파트 간 안전 쿨다운(15초) 적용 (`summarizer.py`)**:
   - `_summarize_chunked_text` 및 `_summarize_chunked_pdf`에서 연속된 파트 호출 사이와 최종 통합(Merge) 직전에 15초 대기(`time.sleep(15.0)`)를 부여하여 분당 토큰 한도(TPM) 안전 리셋 및 성공률 극대화.
3. **단위 테스트 추가 (`test_pdf_processor.py`)**:
   - 3권 이상 장편 시리즈 판별 로직 및 텍스트 분할 청킹 단위 테스트 추가.
4. **가이드 문서 최신화 (`README.md`)**:
   - 장편 시리즈 최적화 및 분할 요약 쿨다운 기능 안내 반영.

### 수정한 파일
- `summarizer.py`: 장편 시리즈 권별 분할 요약 파이프라인 및 분할 요약 파트 간 쿨다운 구현
- `test_pdf_processor.py`: 장편 시리즈 및 분할 요약 단위 테스트 추가
- `README.md`: 기능 안내 갱신
- `CHANGELOG.md`: 변경 기록 추가

### 테스트 결과
- `test_pdf_processor.py` 단위 테스트 전체 정상 통과 (장편 시리즈 분기, 분할 청킹, 텍스트 추출, 볼드 공백, 물결표 이스케이프 등 전체 PASS)
...
### 변경 목적
- 이미 추출된 도서 원문 마크다운 파일(`{도서명}_source.md`)이 디스크에 존재할 경우, 중복 PDF 텍스트 추출 과정을 생략하고 파일에서 즉시 원문 텍스트를 로드하여 처리 속도를 높이고 불필요한 연산을 방지.
- 서평 마크다운(`_review.md`)이 이미 생성되어 스킵되는 도서라도, `--source` 옵션이 켜져 있고 `_source.md`가 누락된 경우 API 호출 없이 로컬에서 원문 마크다운을 즉시 추출하여 산출물을 온전하게 보충.

### 주요 결정 사항
1. **`_source.md` 텍스트 캐싱 로드 (`summarizer.py`)**:
   - `summarize_series_to_markdown`에서 `output_source_md_path` 파일이 존재하고 크기가 유효하면 파일에서 직접 텍스트를 로드(`[⚡ 캐시 활용]`).
   - 이미 캐시 파일이 존재하는 경우 신규 파일 쓰기(`open(..., "w")`)를 생략하여 디스크 I/O 최적화.
2. **스킵 도서의 `_source.md` 자동 보충 생성 (`main.py`)**:
   - 도서 스킵 분기에서 `--source` 옵션이 활성화되어 있고 `_source.md` 파일이 없는 경우, 로컬에서 PDF 마크다운을 추출하여 파일 자동 보충.
3. **단위 테스트 추가 (`test_pdf_processor.py`)**:
   - 기존 `_source.md` 파일 캐시 우선 로드 검증 단위 테스트 추가.
   - 스킵 도서 대상 `_source.md` 자동 보충 생성 검증 단위 테스트 추가.
4. **가이드 문서 동기화 (`README.md`)**:
   - 텍스트 캐싱 및 스킵 도서 리소스 자동 보충 기능 안내 반영.

### 수정한 파일
- `summarizer.py`: `_source.md` 캐싱 로드 로직 구현
- `main.py`: `extract_markdown_from_pdf` 연동 및 스킵 시 `_source.md` 자동 보충 로직 구현
- `test_pdf_processor.py`: 캐시 로드 및 보충 생성 단위 테스트 추가
- `README.md`: 기능 안내 갱신
- `CHANGELOG.md`: 변경 기록 추가

### 테스트 결과
- `test_pdf_processor.py` 단위 테스트 전체 정상 통과 (캐시 우선 로드, 스킵 보충 생성, QuotaExhaustedError, CLI 인자, 마크다운 추출 등 모두 PASS)
